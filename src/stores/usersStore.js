...
const useUsersStore = defineStore("users", () => {
  ...
  const countryList = computed(() => {
    return userList.value.reduce((accumulator, user) => {
      const countryTag = user.tags?.find((tag) => tag.startsWith("Nationality_"));
      
      if (countryTag && !accumulator.includes(countryTag.replace("Nationality_",""))) {
  ...
  const companyList = computed(() => {
    return userList.value.reduce((accumulator, user) => {
      ...
      if (companyTag && !accumulator.includes(companyTag.replace("Company_", ""))) {
...
return{userList, countryList, companyList, totalUsers, genderStats, nationalityStats}; 
}
)
