...
const handleSearch = () => {
  ...
  criteria.nation = filterNation.value;
};
...
const filteredUsers = computed(() => {
  ...
  // search by user name (instant filtering)
  if (criteria.name) {
  ...
  if (criteria.time?.[0] && criteria.time?.[1]) {
    ...
    newUsers = newUsers.filter((user) => {
      const userTime = user.timeUse.split(" ")[1].substring(0, 5); // Extracts HH:mm
...
<template>
  <div class="filter-wrapper">
    ...
    <a-form layout="vertical" class="filter-form">
      <a-row :gutter="[24, 16]">
        <!-- Row 1: Primary Search (Stretched at the top) -->
        <a-col :span="24">
          <a-row :gutter="24">
            <a-col :xs="24" :md="12">
